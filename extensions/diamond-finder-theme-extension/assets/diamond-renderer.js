...
if (typeof window !== "undefined") {
  window.DiamondRenderer = {
    ...
    createDiamondCard(diamond) {
      ...
      const detailsData = [];
      if (diamond.color)
        detailsData.push({ label: "Colour:", value: diamond.color });
      if (diamond.clarity)
        detailsData.push({ label: "Clarity:", value: diamond.clarity });
      if (diamond.cutGrade)
        detailsData.push({ label: "Cut:", value: diamond.cutGrade });
    ...
    renderDiamonds(diamondsToRender) {
      ...
      if (!sortedDiamonds || sortedDiamonds.length === 0) {
        gridArea.innerHTML =
          '<p class="tw-text-center tw-text-gray-500 tw-py-10">No diamonds match your criteria.</p>';
      } else {
